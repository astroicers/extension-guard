...
describe('EG-CRIT-002: Remote Code Execution', () => {
  ...
  it('should detect eval()', () => {
    const files = new Map([['src/extension.js', 'const result = eval(userInput);']]);
  ...
  it('should detect new Function()', () => {
    const files = new Map([['src/extension.js', 'const fn = new Function("return " + code);']]);
    const evidences = critRemoteExecution.detect(files, mockManifest);
    expect(evidences.length).toBeGreaterThan(0);
  ...
  it('should detect child_process.exec', () => {
    const files = new Map([['src/extension.js', `require('child_process').exec('rm -rf /');`]]);
    const evidences = critRemoteExecution.detect(files, mockManifest);
    expect(evidences.length).toBeGreaterThan(0);
  ...
  it('should detect vm.runInContext', () => {
    const files = new Map([['src/extension.js', 'vm.runInContext(code, context);']]);
    const evidences = critRemoteExecution.detect(files, mockManifest);
    expect(evidences.length).toBeGreaterThan(0);
  });
